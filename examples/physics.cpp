...
std::vector<Node> generateRandomNodes(std::size_t n)
{
    ...
    std::uniform_real_distribution<>originDistribution(0.0f, 1.0f);
    std::uniform_real_distribution<>sizeDistribution(0.0f, 0.01f);
    ...
    for (auto i = std::size_t(0); i < n; ++i)
    {
        ...
        nodes[i].box.width = std::min(1.0f - nodes[i].box.left, (float)sizeDistribution(generator));
        nodes[i].box.height = std::min(1.0f - nodes[i].box.top, (float)sizeDistribution(generator));
        nodes[i].id = i;
    }
    return nodes;
}
...
int main()
{
    ...
    std::uniform_int_distribution<> deathDistribution(0,1);
